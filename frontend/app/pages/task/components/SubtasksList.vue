...
const canEditSubtask = (subtask: any) => {
    return (
        props.userRole === "manager" || props.userId === subtask.created_by_uid
    );
};
        ...
        <CardContent>
            <div v-if="subtasks && subtasks.length > 0" class="space-y-2">
                <Collapsible
                    ...
                    <div class="flex items-start gap-3 p-4">
                        <CollapsibleTrigger as-child>
                            <Button
                                variant="ghost"
                                size="sm"
                                class="h-7 w-7 p-0 shrink-0 mt-0.5 hover:bg-slate-100 dark:hover:bg-slate-800">
                                <ChevronDown
                                    class="h-4 w-4 transition-transform duration-200"
                                    :class="{
                                        '-rotate-90': !expandedSubtasks.has(
                                            sub.id
                                        )
                                    }" />
                            </Button>
                        ...
                        <div class="flex-1 min-w-0 space-y-2">
                            ...
                            <div
                                ...
                                <div
                                    v-if="
                                        sub.priorityLevel !== undefined &&
                                        sub.priorityLevel !== null
                                    "
                                    class="flex items-center gap-1">
                                    <Flag class="w-3 h-3" />
                                    <span
                                        >Priority: {{ sub.priorityLevel }}</span
                                    >
                                </div>
                                <LabelBadge
                                    v-if="sub.label"
                    ...
                    <CollapsibleContent>
                        <div
                            class="px-4 pb-4 pt-2 border-t border-slate-100 bg-slate-50/50 dark:border-slate-800 dark:bg-slate-800/50">
                            <div class="pl-10 space-y-3">
                                <div v-if="sub.desc" class="space-y-1">
                                    ...
                                    >
                                    <p
                                        class="text-sm text-slate-700 leading-relaxed dark:text-slate-300">
                                        {{ sub.desc }}
                                    </p>
                                </div>
            ...
            <div
                ...
                <div
                    class="rounded-full bg-slate-100 p-3 mb-3 dark:bg-slate-800">
                    <Circle
                ...
                <p class="text-xs text-slate-500 mt-1 dark:text-slate-500">
                    Subtasks will appear here when created
                </p>
            </div>
        </CardContent>
    </Card>
