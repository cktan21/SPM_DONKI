<template>
  <div class="space-y-4">
    <div
      ...
      class="group cursor-pointer bg-white border border-slate-200 rounded-xl p-6 hover:shadow-lg hover:border-blue-300 transition-all"
    >
      <div class="flex items-start gap-6">
        <!-- Status Bar -->
        <div class="flex-shrink-0 w-1 self-stretch rounded-full" :class="getStatusColor(task.status)"></div>
        ...
        <div class="flex-1 min-w-0">
          <div class="flex items-start justify-between gap-6 mb-4">
            <div class="flex-1 min-w-0">
              ...
              <p v-if="task.desc" class="text-sm text-slate-500 line-clamp-2">{{ task.desc }}</p>
            </div>
            
            <!-- Priority Badge -->
            <div
              :class="[
                'w-14 h-14 rounded-xl flex flex-col items-center justify-center font-bold text-lg shadow-sm flex-shrink-0',
...
<script setup>
import { formatDate, getStatusColor, getStatusBadgeClass, getPriorityColor } from '../utils.js'
