...
const stats = computed(() => {
  const tasks = currentView.value === 'personal' ? filteredTasks.value : allTeamTasks.value
  return {
    total: tasks.length,
...
const filteredTasks = computed(() => {
  ...
  if (filterPriority.value !== 'all') {
    if (filterPriority.value === 'high') {
      ...
      tasks = tasks.filter(t => t.priorityLevel >= 4 && t.priorityLevel < 8)
    } else {
    ...
    tasks: member.tasks.filter(task => {
      ...
      if (filterPriority.value !== 'all') {
        if (filterPriority.value === 'high') {
          ...
          match = match && task.priorityLevel >= 4 && task.priorityLevel < 8
        } else {
...
const fetchData = async () => {
  ...
  try {
    ...
    const response = await fetch(`http://127.0.0.1:4100/uid/${userId.value}`)
